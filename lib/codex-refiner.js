...
async function refinePromptBundle(bundle, brief) {
  ...
  const compactBrief = {
    ...
    dialogueLanguage: brief.dialogueLanguage,
    dialogueVoice: brief.dialogueVoice,
    dialogueScript: truncateText(brief.dialogueScript, 500),
  };
  const localDrafts = {
  ...
  const promptText = [
    ...
    "If bgmRestricted is true, avoid background music and keep only voice and environmental sound design.",
    "If dialogue is enabled: Veo can carry direct dialogue instructions, Runway should treat dialogue as Lip Sync handoff rather than stuffing long speech into the visual prompt, and Kling should keep native-audio dialogue short and speaker-clear.",
    "",
    "Brief:",
